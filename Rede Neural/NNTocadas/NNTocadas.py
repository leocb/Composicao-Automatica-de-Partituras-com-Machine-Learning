...
#TensorBoard
tbCallBack = TensorBoard(log_dir='./../logs/NotasTocadas/', histogram_freq=0, write_graph=True)
#fazer em 34 épocas
#Modelo
#Tipo:
model = Sequential()
...
model.save(ModelsMainPath + "NotasTocadasMain.h5")

#Load Data using numpy
inputData = np.loadtxt(open(DataMainPath+'FFT.data', 'rt'), delimiter=",")
outputData = np.loadtxt(open(DataMainPath+'NotasTocadas.data', 'rt'), delimiter=",")
...
mcCallBack = ModelCheckpoint(ModelsMainPath + "NotasTocadasMain.h5", monitor='val_loss', verbose=0, save_best_only=False, save_weights_only=False, mode='auto', period=1)

# Train the model, 20% of data is used for evaluation
model.fit(inputData, outputData, validation_split=0.10, epochs=10, batch_size=32, callbacks=[tbCallBack,mcCallBack])
